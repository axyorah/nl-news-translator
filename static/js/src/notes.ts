import { JSONData, postData } from './utils';
...
interface TagResponse extends JSONData {
    tag: Tag
}

interface ErrorResponse extends JSONData {
    errors: string
}
...
const tagsUl: HTMLUListElement = document.querySelector('#id_tags');

function createPartialTag(name: string): Promise<TagResponse | ErrorResponse> {
    const data = {
        name: name
    }
...
function addTagToForm(tag: Tag): void {
...
addPartialTagBtn.addEventListener('click', function(evt): void {
    ...
    const name: string = addPartialTagName.value;
    if (!name) {
        throw new Error('No tag name specified!');
    }

    createPartialTag(name)
    .then((res: TagResponse | ErrorResponse) => {
        console.log(res);
